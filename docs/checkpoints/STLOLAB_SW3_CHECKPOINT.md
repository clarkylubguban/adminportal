...
Acceptance stopped before checkout gates were opened. The deployed storefront creates checkout idempotency keys internally and exposes no supported test/operator control for the checkpoint's exact keys. Admin payment and handover controls likewise use their private session retry store rather than an operator field. The runbook required controlled pre-seeding of those retry records and explicitly required stopping if that setup could not be verified. Browser security rejected JavaScript-URL storage access and prohibited indirect execution or another browser workaround. No token, credential, browser claim, direct endpoint, raw RPC, mock control, or alternate key was used.
