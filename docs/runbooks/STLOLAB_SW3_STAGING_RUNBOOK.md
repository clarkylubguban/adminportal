...
Security review is required before enabling these two flags for live staging acceptance. It must confirm exact-staging exclusion, no token or secret exposure, unchanged mutation authorization, same-origin protections, payload-conflict behavior, and fail-closed persistence for operator-supplied keys. Rollback is configuration-only after the acceptance window: close all creation gates first, set both acceptance-key flags to `false` or remove them, then republish/redeploy the same tested code if required by each platform. No migration or data rollback belongs to this control.
