...
## Durable order-access staging step (local design only)

This step is not authorized for staging yet. After the previously applied checkout permission correction, apply only `20260917034542_stlolab_order_access_lifecycle.sql` from Admin commit `596d850bf9239d99e741b379eaaa6e9a5f7d0087`. It is additive, has no data-update statement, and must leave every existing order with `confirmation_access_version is null`; specifically, do not alter or reissue access for `TRRY-ORD-8B12C7F9`.

Before a Storefront update, approve an explicit Preview value for `STLO_ORDER_ACCESS_TTL_SECONDS`. The local fixture value `2592000` means 30 days and is not an approved deployed policy. Keep `STLO_STOREFRONT_GATEWAY_SECRET`, `VERCEL_AUTOMATION_BYPASS_SECRET`, and every access token server-only. Deploy Storefront commit `8e62bb59612bf011c6de014718d73b49ff5948ce` only after separate approval; leave `STLO_CHECKOUT_ENABLED=false` during migration and read-only confirmation checks.

Post-migration read-only checks: verify the four new columns are nullable; the deferred constraint trigger exists; browser roles have no new schema/function grants; the eight-argument create overload is executable only by `service_role`; and the existing staging order remains version-null with the same token hash. A disposable new-order acceptance must separately prove persistent cookie access after browser restart, two-order access, interrupted-response recovery, duplicate submission, expiry/revocation denial, wrong/no-cookie denial, Origin/CSRF denial, and one-time cancellation release. Close creation gates in `finally` as before.

Rollback before any accepted new order is to keep creation disabled, roll the Storefront back to its prior private version, and drop only the new overload, trigger/function, constraint, and four columns after confirming no version-1 rows exist. Once any version-1 order exists, do not drop the lifecycle state; gate creation and prepare a forward correction instead.

## Staging acceptance-key controls

Tested local implementation identities: Admin `22ad034ef5508b33ad44f75ba6423a19533ac298`; Storefront `56a451910747545f4a51c032be4299c6018871cb`.

The documented checkout and Admin action keys are non-secret test identifiers. Do not pre-seed browser storage, extract tokens, inject requests, or bypass the application UI. A reviewed staging-only release must expose the exact-key controls through the normal checkout and authenticated Admin Orders forms. Enable them only when the Storefront runtime has both `STLO_ENV=staging` and `STLO_ACCEPTANCE_KEYS_ENABLED=true`, and the Admin Preview has both `VITE_APP_ENV=staging` and `VITE_STLO_ACCEPTANCE_KEYS_ENABLED=true`. Production and every non-staging environment remain excluded even if the feature flag is set. Blank fields preserve normal automatic key generation.

Enter the documented key in the visible staging field immediately before the normal protected submit. The application validates the existing 16-120 character format, binds the key to the normalized action payload, and persists its own retry record. Reloads and uncertain retries reuse that record; conflicting reuse is rejected before mutation and remains protected by the canonical database idempotency contract. Checkout continues through `/api/checkout/prepare` and `/api/checkout` with the existing same-origin CSRF and HttpOnly token lifecycle. Admin payment and handover continue through the existing authenticated `/api/orders/:id/actions` route and canonical RPCs. If a response is uncertain, inspect canonical state before replaying the same visible key.

Security review passed after the server-side forward corrections in Admin `bfd63e7fd1df8715f5a2364e7e1858cd6cf3d40f` and Storefront `5abb4327274709b84aec7bdcaaf67185adfaa5b2`; these supersede the earlier implementation identities for release. The reviewed control reserves `SW3-BATCH-*`, enforces exact-staging configuration on both servers, exposes no token or secret, leaves mutation authorization unchanged, preserves same-origin protections and payload conflicts, and fails closed when an operator key cannot be persisted.

Deploy both reviewed commits with the acceptance flags false or absent and every creation gate closed. Verify the UI fields are absent and direct namespaced requests are rejected before any cookie, checkout forwarding, or Admin RPC. Only after that read-only deployment verification and separate mutation approval, set the flags true in staging scope and rebuild/republish as required. Rollback is configuration-only: close creation gates first, set both flags false or remove them, redeploy/republish the same reviewed code if an environment revision requires it, and verify direct namespaced requests return `403`. No migration or data rollback belongs to this control.
