...
const PAYMENT_CONFIRMATION_WRITE_ROLES = new Set(["owner", "admin"]);
const PAYMENT_CONFIRMATION_SELECT = [
  "id", "quoted_amount", "amount_due", "quote_status", "payment_status", "payment_method",
  "payment_type", "payment_selected_amount", "payment_reference", "payment_customer_note",
  "payment_verified_amount", "payment_verified_at", "payment_verified_by", "payment_confirmed_amount",
  "payment_confirmed_at", "payment_confirmed_by", "payment_internal_note", "payment_review_note",
  "payment_rejected_at", "payment_history",
].join(",");
...
export default async function handler(request, response) {
  ...
  if (request.method === "POST" && getWorkflowAction(request) === "payment-confirmations") {
    return handlePaymentConfirmationRequest(request, response);
  }
...
async function handlePaymentConfirmationRequest(request, response) {
  const inquiryReference = getInquiryReference(request);
  if (!/^[A-Z0-9][A-Z0-9_-]{2,79}$/.test(inquiryReference)) return sendJson(response, 400, { ok: false, error: "invalid inquiry reference" });
  ...
  try {
    ...
    const adminUser = await getAuthorizedAdmin(supabase, token);
    if (!adminUser) return sendJson(response, 401, { ok: false, error: "admin session required" });
    if (!PAYMENT_CONFIRMATION_WRITE_ROLES.has(adminUser.role)) return sendJson(response, 403, { ok: false, error: "owner or admin access required" });
    ...
    if (!result.ok) return sendJson(response, 400, { ok: false, error: result.error });
    if (result.idempotent) return sendJson(response, 200, { ok: true, inquiry: toPaymentClientInquiry(inquiry), idempotent: true });
    ...
    return sendJson(response, 200, { ok: true, inquiry: toPaymentClientInquiry(updated), idempotent: false });
  } catch (error) {
...
function toPaymentClientInquiry(row) {
  return {
    id: row.id,
