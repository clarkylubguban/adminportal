...
assert.ok(barcodeUi.includes("requestAnimationFrame"), "enhancement scheduling must use browser frame coalescing");
assert.ok(barcodeUi.includes("enhancementFrameId") && barcodeUi.includes("enhancementRunning") && barcodeUi.includes("enhancementQueued"), "enhancement scheduling must be single-flight/coalesced");
assert.equal(barcodeUi.includes("queueMicrotask(() => enhanceM4"), false, "enhancement scheduling must not use unconditional microtask recursion");
assert.ok(/feedback && feedback\.textContent !== nextFeedback[\s\S]*feedback\.textContent = nextFeedback/.test(barcodeUi), "inventory feedback text updates must be guarded by value comparison");
assert.equal(/if \(feedback\) feedback\.textContent = state\.inventoryFeedback \|\| "Inventory scan is read-only\.";/.test(barcodeUi), false, "old unconditional inventory feedback mutation must be absent");
assert.ok(/input && input\.placeholder !== INVENTORY_SEARCH_PLACEHOLDER[\s\S]*input\.placeholder = INVENTORY_SEARCH_PLACEHOLDER/.test(barcodeUi), "inventory search placeholder updates must be idempotent");
assert.ok(/stockCount && stockCount\.title !== STOCK_COUNT_SCANNER_TITLE[\s\S]*stockCount\.title = STOCK_COUNT_SCANNER_TITLE/.test(barcodeUi), "stock count scanner title updates must be idempotent");
...
assert.equal(/printWindow\.document\.close\(\);\s*printWindow\.focus\(\);\s*printWindow\.print\(\);/.test(barcodeUi), false, "print path must not immediately focus/print after document close");
assert.ok(barcodeUi.includes("async function waitForPrintWindowReady(printWindow, expectedLabels)"), "print readiness gate missing");
assert.ok(barcodeUi.includes("doc.readyState === \"complete\""), "print readiness must wait for document completion");
assert.ok(barcodeUi.includes("doc.querySelectorAll(\".label\")"), "print readiness must verify expected labels");
assert.ok(barcodeUi.includes("doc.querySelectorAll(\".label svg\")"), "print readiness must verify label SVGs");
assert.ok(barcodeUi.includes("getBoundingClientRect()"), "print readiness must inspect rendered SVG dimensions");
assert.ok(barcodeUi.includes("rect.width > 0 && rect.height > 0"), "print readiness must require non-zero SVG dimensions");
assert.ok((barcodeUi.match(/await nextPrintFrame\(printWindow\);/g) || []).length >= 2, "print readiness must await two paint frames");
assert.ok(barcodeUi.includes("const deadline = Date.now() + 3000"), "print readiness must be bounded by a timeout");
assert.ok(barcodeUi.includes("Print preview could not finish rendering. Please try again."), "print readiness failure feedback missing");
assert.ok(/const ready = await waitForPrintWindowReady\(\s*printWindow,\s*labels\.length\s*\);[\s\S]*if \(!ready\)[\s\S]*printWindow\.focus\(\);[\s\S]*printWindow\.print\(\);/.test(barcodeUi), "print must happen only after readiness succeeds");
