...
const productInfoBlock = main.match(/function renderCatalogEditorProductInformation[\s\S]+?function renderCatalogEditorImages/)?.[0] ?? "";
...
assert.equal(productInfoBlock.indexOf('renderCatalogField("productCode"') < productInfoBlock.indexOf('renderCatalogField("salesChannels"'), true, "Sales Channels render directly after Product Code");
...
assert.equal(styles.includes(".catalog-product-editor-page .catalog-field-salesChannels"), true, "Sales Channels field spans the Product Information form width");
...
console.log(JSON.stringify({ result: "master_catalog_sales_channels_ok", assertions: 24 }, null, 2));
