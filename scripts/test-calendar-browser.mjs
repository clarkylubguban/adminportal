...
try {
  ...
  await waitForText(cdp, desktop, "TASK DEADLINE");
  ...
  await waitForText(cdp, desktop, "No dated tasks");
  await click(cdp, desktop, "[data-calendar-prev]");
  await waitForText(cdp, desktop, "August 2026");

  await navigate(cdp, desktop, `http://127.0.0.1:${port}/calendar?qaRole=staff`);
  await waitForText(cdp, desktop, "Synthetic Staff A");
  await assertEval(cdp, desktop, `document.body.innerText.includes('Synthetic Staff B private task') === false`, "staff calendar cannot see other assignee");
  assert.equal(await evalValue(cdp, desktop, `fetch('/api/task-calendar?assignedUserId=${staffB}', { headers: { Authorization: 'Bearer synthetic-staff-a-token' } }).then((response) => response.json()).then((payload) => payload.events.length)`), 0, "staff assignee spoof did not reveal events");

  await navigate(cdp, desktop, `http://127.0.0.1:${port}/calendar?qaRole=admin`);
  await waitForText(cdp, desktop, "Synthetic Staff B private task");
