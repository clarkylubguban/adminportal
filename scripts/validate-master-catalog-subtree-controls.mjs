...
const checks = [
  ...
  ["Update Price uses existing catalog product update contract", main.includes("function updateCatalogQuickPrice") && main.includes("updateAdminCatalogProduct(productId") && main.includes("data-catalog-quick-selling-price")],
  ["Update Image uses supported catalog image persistence", main.includes("function updateCatalogQuickImage") && main.includes("uploadCatalogImage(file, product, adminAuthSession)") && main.includes("deleteCatalogImageByUrl")],
  ["Duplicate uses existing create contract", main.includes("function duplicateCatalogProduct") && main.includes("createAdminCatalogProduct(copy, adminAuthSession)")],
