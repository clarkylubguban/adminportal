...
const container = `trry-product-cutover-validation-${Date.now()}`;
...
try {
  await run("docker", [
    "run", "--name", container, "-e", `POSTGRES_PASSWORD=${password}`,
    "-v", `${dockerPath(workspace)}:/workspace:ro`, "-d", image,
  ], { capture: true });
  await waitForPostgres();
  await psqlStdin(bootstrap);
  await psqlFile("/workspace/supabase/migrations/202607110001_create_catalog_products.sql");
  await psqlFile("/workspace/supabase/migrations/202607120001_create_catalog_images_bucket.sql");
  ...
  await psqlFile("/workspace/supabase/migrations/202608140001_canonical_master_catalog_product_cutover.sql");
  await psqlFile("/workspace/supabase/tests/master_catalog_product_cutover.sql");
  console.log("Master Catalog canonical product cutover validation passed.");
} finally {
