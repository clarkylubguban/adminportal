-- Phase 9B-A: durable Quality Check metadata on the existing ops_inquiries
-- compatibility surface. This keeps Production lifecycle state narrow while
-- allowing history to distinguish QC entry and successful QC completion.

alter table public.ops_inquiries
  add column if not exists qc_started_at timestamptz,
  add column if not exists qc_started_by uuid,
  add column if not exists qc_note text,
  add column if not exists qc_completed_at timestamptz,
  add column if not exists qc_completed_by uuid;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'ops_inquiries_qc_started_by_fkey'
      and conrelid = 'public.ops_inquiries'::regclass
  ) then
    alter table public.ops_inquiries
      add constraint ops_inquiries_qc_started_by_fkey
      foreign key (qc_started_by)
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'ops_inquiries_qc_completed_by_fkey'
      and conrelid = 'public.ops_inquiries'::regclass
  ) then
    alter table public.ops_inquiries
      add constraint ops_inquiries_qc_completed_by_fkey
      foreign key (qc_completed_by)
      references public.admin_users(user_id)
      on delete set null;
  end if;
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'ops_inquiries_qc_note_length_check'
      and conrelid = 'public.ops_inquiries'::regclass
  ) then
    alter table public.ops_inquiries
      add constraint ops_inquiries_qc_note_length_check
      check (qc_note is null or char_length(qc_note) <= 500);
  end if;

  if not exists (
    select 1
    from pg_constraint
    where conname = 'ops_inquiries_qc_completion_after_start_check'
      and conrelid = 'public.ops_inquiries'::regclass
  ) then
    alter table public.ops_inquiries
      add constraint ops_inquiries_qc_completion_after_start_check
      check (
        qc_completed_at is null
        or (qc_started_at is not null and qc_completed_at >= qc_started_at)
      );
  end if;
end $$;

create index if not exists ops_inquiries_qc_started_at_idx
  on public.ops_inquiries (qc_started_at)
  where qc_started_at is not null;

create index if not exists ops_inquiries_qc_completed_at_idx
  on public.ops_inquiries (qc_completed_at)
