-- Phase 13-R1: decouple native TRRY Order lifecycle guards from legacy Odoo SO identity.
-- Native confirmation is proven by public.orders.source_inquiry_id; legacy compatibility
-- remains valid when an historical Odoo SO is present.
...
as $$
  select
    ...
    or nullif(btrim(coalesce(legacy_odoo_so, '')), '') is not null
$$;
...
begin
  ...
  if new_status = 'won' and old_status is distinct from 'won' then
    if old_status in ('lost', 'cancelled', 'canceled')
      ...
      raise exception using errcode = '23514', message = 'Order conversion requires quote approval, a positive quote, and a confirmed native or legacy order.';
    end if;
  end if;
  ...
  if new.production_stage is distinct from old.production_stage then
    if new_status in ('lost', 'cancelled', 'canceled')
      ...
      or lower(coalesce(new.quote_status, '')) <> 'approved'
      or not public.trry_ops_inquiry_has_order_confirmation(new.id, new.odoo_so) then
      raise exception using errcode = '23514', message = 'Production requires a confirmed, non-cancelled native or legacy order.';
    end if;
