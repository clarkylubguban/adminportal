-- Phase 13-R5: payment confirmation is an Order-owned action.
-- Keep public.orders as the only active Order authority, remove stale
-- Inquiry receipt/artwork prerequisites from payment confirmation itself,
-- and preserve release-to-production readiness gates.
...
declare
  ...
  payment_actor_id uuid := coalesce(new.payment_confirmed_by, new.payment_verified_by);
begin
  ...
  if lower(coalesce(new.payment_status, '')) in ('confirmed', 'paid')
    ...
    if lower(coalesce(new.quote_status, '')) <> 'approved'
      ...
      or payment_actor_id is null
      or not exists (
        select 1
        from public.admin_users
        where user_id = payment_actor_id
          and is_active = true
          and role in ('owner', 'admin')
      ) then
      raise exception using errcode = '23514', message = 'Payment confirmation requires an approved quote, native TRRY Order, positive amount, confirmation timestamp, and owner/admin actor.';
    end if;
  end if;
