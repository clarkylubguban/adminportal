-- Align the existing ops_inquiries aggregate with the approved Admin Portal MVP.
-- All columns remain nullable. Existing rows and status values are not rewritten.
...
alter table public.ops_inquiries
  ...
  check (
    ...
    or production_stage in (
      ...
      'ready',
      'completed',
      -- Compatibility values already used by the current Admin Portal.
      'in_production',
      'qc_finishing',
      'ready_for_fulfillment'
    )
  );
-- Enforce the locked MVP workflow even when an authenticated client writes directly.
