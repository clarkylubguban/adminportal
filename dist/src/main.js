...
function renderCustomersPage() {
  ...
  return `<main class="customers-page">
    <header class="customers-header"><div><nav>Home <span>/</span> Customers</nav><h1>Customers</h1><p>Customer contacts across your sales channels.</p></div><button class="customer-primary-button" data-customer-add type="button">Add customer</button></header>
    ...
    <section class="customer-metrics">
      ...
      ${renderCustomerMetric("STLO WEB", stats.stlo, "First source", "package")}
    </section>
    ${showFilterTools ? `<section class="customer-toolbar"><label><span>Search customers</span><input id="customer-search" type="search" placeholder="Search name, mobile, or reference" value="${escapeHtml(customerSearch)}"></label><span class="customer-result-count">${rows.length} of ${summaryLabel}</span></section>` : ""}
    ${customerLoadState === "loading" ? `<section class="customer-empty"><h2>Loading customers...</h2></section>` : customerLoadError ? `<section class="customer-empty error"><h2>Customers unavailable</h2><p>${escapeHtml(customerLoadError)}</p><button data-customer-retry type="button">Try again</button></section>` : rows.length ? `<section class="customer-table-wrap"><table class="customer-table"><thead><tr><th>CUSTOMER</th><th>CONTACT</th><th>SOURCE</th><th>STATUS</th><th>ACTION</th></tr></thead><tbody>${rows.map((customer) => `<tr><td data-label="Customer"><strong>${escapeHtml(customer.full_name)}</strong><small>${escapeHtml(customer.customer_reference)}</small></td><td data-label="Contact">${escapeHtml(customer.mobile_normalized || customer.mobile_raw)}</td><td data-label="Source"><strong>${escapeHtml(formatCustomerSource(customer.first_source))}</strong><small>${escapeHtml(formatCustomerDate(customer.first_seen_at || customer.created_at))}</small></td><td data-label="Status"><span class="customer-status ${customer.active === false ? "inactive" : ""}">${customer.active === false ? "Inactive" : "Active"}</span></td><td data-label="Action"><button data-customer-view="${escapeHtml(customer.id)}" type="button">View</button></td></tr>`).join("")}</tbody></table><footer>Showing ${rows.length} of ${customers.length} customers</footer></section>` : `<section class="customer-empty"><div><span>CUSTOMERS</span><h2>${query ? "No matching customer" : "No customer records yet"}</h2><p>${query ? "Try another name, mobile number, or customer reference." : "Add your first customer to keep their contact details in one place."}</p>${query ? "" : `<button class="customer-primary-button" data-customer-add type="button">Add customer</button>`}</div></section>`}
