...
function renderMvpPaymentConfirmation(item) {
  ...
  const acceptanceKeyField = isRetail && isStlolabAcceptanceKeyControlEnabled(window.TRRY_ADMIN_ENV)
    ? `<label class="wide"><span>Staging acceptance key <small>(optional)</small></span><input data-mvp-payment-field="acceptanceIdempotencyKey" maxlength="120" pattern="SW3-BATCH-[A-Z0-9_-]{6,110}" autocomplete="off" ${isLoading ? "disabled" : ""} /><small>Blank keeps automatic retry-key generation.</small></label>`
    : "";
...
function renderMvpOrderFulfillment(item) {
  ...
  const acceptanceKeyField = isStlolabAcceptanceKeyControlEnabled(window.TRRY_ADMIN_ENV)
    ? `<label class="mvp-acceptance-key"><span>Staging acceptance key <small>(optional)</small></span><input data-mvp-fulfillment-idempotency-key maxlength="120" pattern="SW3-BATCH-[A-Z0-9_-]{6,110}" autocomplete="off" /><small>Blank keeps automatic retry-key generation.</small></label>`
    : "";
